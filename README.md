...
A: SQL is a language which is used to query the relational database. RDBMS is the software that manages storage, updating, deleting, and querying.

1. Why do tables need a `primary key`?

A: Tables need a primary key that can be linked to when referring to related content from another table. It also serves as an easy to use lookup value because it is a unique identifier.
